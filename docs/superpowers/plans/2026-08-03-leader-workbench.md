...
- [ ] 写入一个重大待交办、一个已闭环、一个履职考核项的失败测试，断言总数、状态数、重大数、履职率和待办排序。
- [ ] 运行 `node --test tests/workbench/summary.test.mjs`，预期模块不存在而失败。
- [ ] 实现纯函数：统计 `REPORTED/ASSIGNED/RECTIFICATION_SUBMITTED/CLOSED`、重大项；履职只读取同一快照的 `completionRate/reviewItems/assessmentCount`；排序为重大、待复核、临期、其余。
- [ ] 运行 `node --test tests/workbench/summary.test.mjs` 及全量 Node 测试，确认通过。
- [ ] 提交：`git commit -m "feat: add leader workbench summary model"`。
...
- [ ] 写失败测试：默认环境关闭角色切换；显式环境开启；工作台快照包含 `MOCK` 数据状态与隐患指标。
- [ ] 运行 `node --test tests/workbench/service-contract.test.mjs`，预期模块不存在而失败。
- [ ] 服务一次读取 Issue 列表和当前用户；履职仪表盘无权限时返回 `null` 而非伪造零值；交给领域模型统一汇总。
- [ ] Profile 仅在本机构建标志开启且当前角色为 `SUPER_ADMIN` 时渲染 Mock 身份切换区。
- [ ] 运行工作台、履职、隐患、会话全量测试并提交：`git commit -m "feat: add workbench snapshot service"`。
...
- [ ] 写失败组件契约测试，断言四个标签和四个 Tab 路径存在。
- [ ] 运行 `node --test tests/navigation/adaptive-navigation.test.mjs`，预期组件不存在而失败。
- [ ] 实现无装饰性图标的文字导航；在 `App.vue` 仅于 H5 宽屏隐藏原生 `.uni-tabbar`；四个 Tab 页引入组件并留出左侧空间。
- [ ] 将 `pages.json` Tab 顺序改为工作台、履职、随手拍、我的，不改路由。
- [ ] 运行导航、工作台和既有全量测试并提交：`git commit -m "feat: add adaptive workbench navigation"`。
...
- [ ] 写失败页面契约测试，断言存在“演示数据”、六项闭环标签、三项履职标签、“重点待办”和 `getWorkbenchSnapshot`。
- [ ] 运行 `node --test tests/workbench/page-contract.test.mjs`，预期当前首页未使用该服务而失败。
- [ ] 以现有深矿物绿工作台体系重写首页；无履职权限显示“当前角色无履职总览权限”；没有真实数据时不产生真实经营结论。
- [ ] 运行 `pnpm test`、`pnpm run build:h5`、`pnpm run build:mp-weixin`、`git diff --check` 和 Impeccable 检测器。
- [ ] 记录开发日志、勾选全部任务并提交：`git commit -m "feat: deliver leader safety workbench"`。
